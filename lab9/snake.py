...
# Функция для генерации случайной позиции еды с разным весом (не на змейке и не на стене)
...
def game():
    ...
    while running:
        ...
        if (head_x, head_y) == (food_x, food_y):
            ...
            food_x, food_y, food_weight, food_time = generate_food(snake)  # Генерируем новую еду
        ...
        # Проверка времени жизни еды (если прошло 5 секунд, еда исчезает)
        if time.time() - food_time > 5:
            food_x, food_y, food_weight, food_time = generate_food(snake)  # Генерация новой еды

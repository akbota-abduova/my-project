...
SCORE = 0  
COINS = 0  
N = 5  # Количество монет для увеличения скорости
...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.weight = random.randint(1, 10)  # Рандомный вес монеты
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, coins_group):
        ...
        for i in coins_group:
            ...
            SCORE += i.weight  # Добавляем вес монеты в счет

        # Увеличиваем скорость врагов при сборе N монет
        if COINS >= N:
            SPEED += 1
            COINS = 0  # Сбрасываем счетчик монет для следующего уровня
